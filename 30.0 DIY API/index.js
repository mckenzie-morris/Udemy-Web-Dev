...
app.get('/random', (req, res) => {
  ...
  return res.status(200).send(jokes[randomIdx]);
});
//2. GET a specific joke ✅
app.get('/jokes/:id', (req, res) => {
  const jokeId = req.params.id;
  for (let elmt of jokes) {
    if (elmt.id === Number(jokeId)) {
      return res.status(200).send(elmt);
    }
  }
  return res.status(400).send({ error: 'Could not find joke with that ID' });
});

//3. GET a jokes by filtering on the joke type ✅
app.get('/filter', (req, res) => {
  const typeOfJoke = req.query.type;
  const returnJokes = jokes.filter((currVal) => {
    if (currVal.jokeType === typeOfJoke) {
      return currVal;
    }
  });
  if (returnJokes.length) {
    return res.status(200).send(returnJokes);
  } else {
    return res.status(400).send({ error: 'No jokes of that type' });
  }
});
